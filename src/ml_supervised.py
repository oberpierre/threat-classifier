...
def intrusion_detection_system(model_path, csv_test_path, start_time=None, end_time=None, selected_features=None):
    ...
    This function generates a list ordered by the dst-IPs/port tuple that are most popular among the flows classified as
    malicious. Also the src IP addresses that are associated with these IP/port tuples are added to the list.
    If the model works as intended, the most popular dst IPs should belong to the C&C servers owned by the red team
    whereas the listed src IPs connecting to them originate from the infected machines in our network.
    ...
    # convert formatted times to utc unix timestamps
...
def intrusion_detection_system_multiple_csv(model_path, csv_directory, selected_features, get_times=True):
    ...
    if get_times and nr_mal_dsts > 0:
        print(
            'First malicious flow seen at {}'.format(datetime.fromtimestamp(start_time).strftime('%d/%m/%Y %H:%M:%S')))
...
def evaluate_multiple_classifiers_by_host(classifiers, datasets_path, selected_features, nr_files_to_evaluate):
    ...
    for name, clf in classifiers.items():
        ...
        start = time.time()
        precision, recall, f1, log_dir = ml_training.train_and_test_multiple_sets(clf, datasets_path,
                                                                                  selected_feature_names=selected_features,
                                                                                  log_suffix=name, balance=False,
                                                                                  standardize=True,
...
def generate_csv_with_attack(csv_path, feature_to_attack, new_values, suffix):
    ...
    """
    out_path = os.path.splitext(csv_path)[0] + '-{}{}.csv'.format(feature_to_attack.replace(" ", "").replace("/", ""),
                                                                  suffix)
...
def generate_csv_with_multiple_attacks(csv_path, features_to_new_values_dict, suffix=''):
    ...
    :return: path to the new .csv file, containing the altered values
    """
    features = [name.replace(" ", "").replace("/", "") for name in features_to_new_values_dict.keys()]
    out_path = os.path.splitext(csv_path)[0] + '-'.join(features) + suffix + '.csv'
...
if __name__ == "__main__":
    # ls17_labelled_path = '/home/ls19/flows/ls17/lite/t2lite_flows-labelled.csv'
    ls17_labelled_path = '/home/ls19/flows/ls17/full/t2full_flows-labelled.csv'

    # ls18_labelled_path = '/home/ls19/flows/ls18/lite/t2lite_flows-labelled.csv'
    # ls18_labelled_path = '/home/ls19/flows/ls18/full/t2full_flows-labelled.csv'

    extracted_path17 = '../data/extracted/ls17'
    extracted_path18 = '../data/extracted/ls18'

    labelled_data_path = ls17_labelled_path

    host_datasets_path = '/home/ls19/flows/ls17/full/10sets-1-0.5-0.5-H-all-full-features-1552307829'
    ...
    # featurename_csv_path = '../data/rfe_removed_features_list-Tranalyzer-lite-numerical.txt'
    featurename_csv_path = '../data/rfe_removed_features_list-Tranalyzer-full-numerical.txt'
    host_split_path = '../data/host_train_test_IP_splits_2905.pickle'

    header = ml_data.full_header_names
    features = ml_data.all_full_features_ordered_by_importance
    ...
    categorical_feature_indices = map(lambda x: header.index(x), categorical_features)
    ...
    rf_tuned = RandomForestClassifier(n_jobs=-1, random_state=0, n_estimators=128, max_depth=10)
    ...
    # ml_helpers.generate_multiple_sets_by_host(10, labelled_data_path, features + ['Label'],
    #                                           subsampling_fraction=1, train_hosts_fraction=0.5,
    #                                           normal_traffic_fraction=0.5,
    #                                           malicious_hosts=malicious_ips_ls17,
    #                                           suffix='H-all-full-features', host_splits=host_splits)
    ...
    # # | STEP-1
    # X, Y, _, _ = ml_helpers.load_dataset(labelled_data_path, subsampling=0.3, train_fraction=1.0,
    #                                      selected_feature_names=features, standardize=False)
    # X, feature_names = ml_helpers.eliminate_features(X, ml_data.tranalyzer_constants
    #                                                  + ml_data.tranalyzer_categorical_features,
    #                                                  features)
    # ml_feature_selection.MI_vs_Corr_vs_DC(X, Y, feature_names)
    # # | STEP-2
    # ml_feature_selection.get_features_ordered_by_score(X, Y, features, 'Tranalyzer-full')

    # ## CREATE FEATURE SELF CORRELATION MATRICES
    # X, Y, _, _ = ml_helpers.load_dataset(labelled_data_path, train_fraction=1.0,
    ...
    #
    _, _, _, feature_count = ml_feature_selection.train_and_test_with_different_feature_subsets(rf_tuned, [50, 30, 20, 15],
                                                                       featurename_csv_path, host_datasets_path,
                                                                       log_suffix='rf-different-feature-counts',
                                                                       nr_files_to_evaluate=None,
                                                                       prob_scores=False, standardize=False,
                                                                       balance=False)
    print('Best use {} features'.format(feature_count))
    ...
    #                                          selected_feature_names=ml_data.all_lite_features + ['Label'],
    ...
    # ml_training.train_model_and_save(rf, '../data/models', model_name='RF_ls17_RF.sav',
    #                                  selected_features=ml_data.all_lite_features + ['Label'],
    ...
    ml_training.train_model_and_save(rf_tuned, './models',
                                     model_name='RF_ls17-RF_' + str(feature_count) + '_full.sav',
                                     selected_features=non_categorical_features[:feature_count] + ['Label'],
                                     csv_train_path=labelled_data_path,
                                     standardize=False, balanced=False)
    # ml_training.train_model_and_save(rf_tuned, './models', #                                  model_name='RF_ls18-RF_top10_noSYN_noPSH_withProtocol_intExt_128est_maxdepth10.sav',
