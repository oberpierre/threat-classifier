...
def train_and_test(classifier, csv_train_path, csv_test_path, selected_feature_names=None, swap_traintest=False, cv_fold_and_repeat=None,
    ...
    if cv_fold_and_repeat != None:
        ...
        if pd.isnull(X_train).any():
            print('Cleansing training data')
            nan_mask = ~pd.isnull(X_train).any(axis=1)
            nan_indices = list(filter(lambda x: x > -1, map(lambda x: -1 if x[1] else x[0], enumerate(nan_mask))))
            print('Rows {} contain NaN values. Removing these rows in X and Y.'.format(nan_indices))

            X_train = X_train[nan_mask]
            Y_train = Y_train[nan_mask]

        if pd.isnull(X_test).any():
            print('Cleansing test data')
            nan_mask = ~pd.isnull(X_test).any(axis=1)
            nan_indices = list(filter(lambda x: x > -1, map(lambda x: -1 if x[1] else x[0], enumerate(nan_mask))))
            print('Rows {} contain NaN values. Removing these rows in X and Y.'.format(nan_indices))
